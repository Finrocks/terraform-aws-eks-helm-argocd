  ...
  argocd_helm_values = templatefile("${path.module}/helm-values/argocd.yaml",
    {
      ...
      controller_role_arn    = local.iam_role_enabled == true ? try(one(module.argocd_application_controller_iam_role[*].service_account_role_arn)) : "sasha holder"
      server_sa_name         = local.server_service_account_name
      server_role_arn        = local.iam_role_enabled == true ? one(module.argocd_server_iam_role[*].service_account_role_arn) : try(one(module.argocd_server_iam_role[*].service_account_role_arn))
