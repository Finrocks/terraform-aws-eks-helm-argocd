  ...
  argocd_helm_values                          = templatefile("${path.module}/helm-values/argocd.yaml",
    {
      ...
      server_role_arn                         = local.iam_role_enabled ? one(module.argocd_server_iam_role[*].service_account_role_arn) : try(one(module.argocd_server_iam_role[*].service_account_role_arn), "")
